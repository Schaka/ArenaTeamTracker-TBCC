...
local MAJOR_VERSION = "LibSimpleOptions-1.01"
local MINOR_VERSION = 90000 
...
local WotLK = not ToggleAchievementFrame
...
--]]	
	...
	-- @usage LibStub("LibSimpleOptions-1.01").AddOptionsPanel("My Options", function(panel) ... end)
	...
	-- @param name name of your panel
	-- @param controlCreationFunc function to call when the panel is first shown
	-- @usage LibStub("LibSimpleOptions-1.01").AddOptionsPanel("My Options", "My Suboptions", function(panel) ... end)
	-- @return the created panel
...
do
	...
	function panelMeta:MakeSlider(...)
		...
		if args.vertical == 1 then
		 slider:SetWidth(20)
		 slider:SetHeight(110)
		...
		if args.currentTextFunc then
			...
			if args.vertical == 1 then
			currentText:SetPoint("TOP", slider, "CENTER", 20, 0) else
			currentText:SetPoint("TOP", slider, "CENTER", 0, -8) end
...
do
	...
	function panelMeta:MakeDropDown(...)
		...
		--local dropDown = CreateFrame("Frame", name, self, "L_UIDropDownMenuTemplate")
		--local dropDown =  L_Create_UIDropDownMenu(name, self)
		local dropDown = MSA_DropDownMenu_Create(name, self)
		...
		if WotLK then
			--L_UIDropDownMenu_SetWidth(dropDown, 100)
		    MSA_DropDownMenu_JustifyText(dropDown, "CENTER")			
		else
	    	--L_UIDropDownMenu_SetWidth(100, dropDown)
			MSA_DropDownMenu_JustifyText(dropDown, "CENTER")			
		end
...
-- @usage LibStub("LibSimpleOptions-1.01").AddSlashCommand("My Options", "/MyOpt", "/MO")
